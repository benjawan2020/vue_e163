...
export default {
  ...
  methods: {
    ...
    formData.append('code',this.pre_name)
    formData.append('code',this.first_name)
    formData.append('code',this.last_name)
    formData.append('code',this.is_active)
    let res = await this.$http.post('http://localhost:7001/addstd', formData)
    //  let res = await fetch('http://localhost:7001/addstd' + this.formData, {
    //     method: 'post',
    //     headers: {
